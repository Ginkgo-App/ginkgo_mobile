  ...
  Widget _buildMessageImages() {
    if (message?.images != null && message.images.isNotEmpty) {
      if (messageImageBuilder != null) {
        return messageImageBuilder(message.images[0].mediumThumb, message);
      } else {
        final images =
            GalleryItem(images: message.images.map((e) => e.original).toList());
        return Padding(
          padding: EdgeInsets.only(top: 5.0, bottom: 5.0),
          child: ClipRRect(
            borderRadius: BorderRadius.circular(10),
            child: SizedBox(
              height: message.images.length > 1
